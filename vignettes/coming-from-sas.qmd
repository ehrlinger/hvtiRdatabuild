...
`exclude`; the `vars` list alone expresses its projection. Add `n_events` and
`n_censored` expectations only when `vars` contains the event column registered
in `_study.yml`; `write_analysis_set()` calculates both from that column.
